...
5. Define the contract first: `Entry`, `Annotation`, `Tag` (`group:value`), the annotation-level typed `Result` (string/number dimensions), `SavedView`/query, `InternalLinkAddress`, `InternalLinkTarget`, `TextLinkSpan`, image-asset ref — types and schema before UI.
...
4. **Annotation tagging & result** — attach group tags to any annotation (e.g. setup) and set an optional typed `result` (user-defined string/number dimensions, stats-only, not browsable); notes are taggable text-box annotations; keep the Annotation-Tag Index (tags + result) in sync.
...
9. **Stable internal links & text hyperlinks** — every Entry/annotation exposes one immutable-id internal address; editable title/text-box character ranges carry typed links to those targets, with create/edit/remove/follow, text-edit transformations, undo/redo, and a dedicated projection. Do not model references as `Annotation.links[]`.
...
- Do not add fallback code, compatibility shims, or dual behavior paths. For the v0.1.0+ persisted contract, append one forward migration and a golden-DB test, then keep only the new runtime model.
...
- Proposed slices:
- First slice:
- Contract changes (Entry / Annotation / Tag / SavedView / InternalLinkAddress / InternalLinkTarget / TextLinkSpan):
